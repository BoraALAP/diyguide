...
alter table "public"."user"
add constraint "unique_email" UNIQUE using index "unique_email";
alter table "public"."user"
add constraint "user_id_fkey" FOREIGN KEY (id) REFERENCES auth.users(id) ON UPDATE CASCADE ON DELETE CASCADE not valid;
...
create policy "allow_inserts_from_auth_trigger" on "public"."user" as permissive for
insert to public with check (true);
create policy "user can get their info" on "public"."user" as permissive for
select to authenticated using ((auth.uid() = id));
create policy "Select Public" on "public"."guides" as permissive for
